...
public enum CopyOfConnectionProvider {
	...
	public synchronized void sendCommand(String command) {
		...
		Log.w(TAG, "command_queue: " + Integer.toString(command_queue.size()));
	}
	
	public boolean isWriting(){
		try {
			Log.w(TAG, "isWriting: " + Integer.toString(command_queue.size()));
			if (command_queue.size() > 0) return true;
			else return false;
	...
	public boolean stillConnected() {
		...
		try {
			...
			Log.w(TAG, "couldn't write HEART");
			status = Status.NO_CONNECTION;
			return false;
		}
	...
	private class ConnectorThread extends Thread {
		...
		protected void finalize() throws Throwable {
			Log.e(TAG, "Finalize the ConnectorThread");
		...
		public void run() {
			...
			try {
				...
				Log.v(TAG, "could/t finalize connectorthread");
			}
		}
